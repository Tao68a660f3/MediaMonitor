...
| `WindowBounds` | `"120,80,900,700"` | 窗口位置与大小 `"left,top,width,height"`，**程序自动写入**（关闭/最小化到托盘时保存，启动时校验后应用；首装无此键） |

> 🪟 **窗口布局**：`WindowBounds` 由程序自动维护。启动时会先校验 —— 保存的位置若已不在当前屏幕内（显示器被拔掉、分辨率变小、被手改到屏外），会**整份忽略**并回到系统默认摆放（只在调试输出 `Debug.WriteLine` 里留一行说明）；尺寸则收敛到 `[400×300, 当前虚拟屏幕]`。手改此项可临时生效，但下次关闭窗口会被实际布局覆盖（自我修复）。
...
* **配置**：`Services/ConfigService` 逐项容错地读写 `config.json`，启动时注入各消费者；静默项的生效时机见上文「⚙️ 配置项生效时机」。
* **UI 辅助**：`UI/LatencyTestController`（测延迟交互）、`UI/WindowPlacement`（窗口布局持久化：关闭时写入、启动时校验后应用）。
